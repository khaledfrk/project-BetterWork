    ...
    <div>
      ...
      <a href="#">
        ...
        <div class="form-group">                    
          ...
          <input v-model="service.numPlaces" type="number" class="form-control mb-4" id="exampleInputPassword6" placeholder="Number Of Places">
          <p class="lead" style="color: grey; font-weight: 300;">Remaining Days:</p>
          <input v-model="duration" type="number" class="form-control mb-4" id="exampleInputPassword7" placeholder="Duration (Days)">
...
export default {
  ...
  name: 'UpdateOffer',
  computed: {
    ...mapGetters(['service', 'token', 'documents', 'value', 'duration']),
    value: {
      get() { return this.$store.getters.value; },
      set(newValue) { this.$store.commit('setValue', newValue); }
    },
    duration: {
      get() { return this.$store.getters.duration; },
      set(newValue) { this.$store.commit('setDuration', newValue); }
    }
    /*value: {
      get(){
        return 'value'
  ...
  created() {
    ...
    //this.handleUncomp()
    //this.handleDocs()
    //this.handleRemainingDays()
  },
  methods: {
    /*handleUncomp() {
      this.docs = this.newValue
      this.rem = this.duration
      console.log(this.docs, this.rem)
    },*/
    ...
    handleUpdateService() {
      ...
      const dur = {
        beginOffer: this.service.duration.beginOffer,
        endOffer: end.addDays(parseInt(this.duration, this.service.duration.beginOffer))
      }
      const updates = {
        ...
        numPlaces: parseInt(this.service.numPlaces),
        duration: dur
      }
      //console.log(updates)
