<template>
    <div>
      ...
      <a href="#">
        <div class="d-flex">
        <p @click="goToOffers()">{{typeOfService.name}}s List</p>
        ...
        <div class="form-group">
            ...
            <div class="row ml-1">
                <div class="input-group col-3" :class="{ invalid: $v.part.$error }"><input v-model="part" type="text" class="form-control mb-4" id="exampleInputPassword2" placeholder="Article number" @blur="$v.part.$touch()">
                </div>
        ...
        <div class="input-group" :class="{ invalid: $v.budget.$error }">
          ...
          <p class="lead" style="color: grey; font-weight: 300;">Number Of Places:</p>
        <div class="input-group" :class="{ invalid: $v.numPlaces.$error }">
          <input v-model="numPlaces"  class="form-control mb-4" id="exampleInputPassword6" placeholder="Number Of Places" @blur="$v.numPlaces.$touch()">
          </div>
          <p class="lead" style="color: grey; font-weight: 300;">Duration:</p>
        <div class="input-group" :class="{ invalid: $v.duration.$error }">
          <input v-model="duration"  class="form-control mb-4" id="exampleInputPassword7" placeholder="Duration (Days)" @blur="$v.duration.$touch()">
          </div>
        <div class="row justify-content-start mb-4 mt-4">
            <button @click="handleAddService" type="submit" class="btn btn-primary pt-2 pb-2 pr-5 pl-5 btn-grad "style="padding:15px 45px;font-size:15px" >Add {{typeOfService.name}}</button>
          </div>
      </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    goToOffers(){
      this.$router.push('/dashboard/services/offers')
    },
    handleAddService() {
      const doc = []
      ...
      const dur = {
        beginOffer: begin,
        endOffer: end.addDays(parseInt(this.duration))
      }
      const data = {
      ...
      //this.$router.go(this.$router.push('/dashboard/services/offers'))
    }
  },
}
</script>
